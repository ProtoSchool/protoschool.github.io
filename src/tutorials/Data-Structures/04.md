...
A `CID` (Content IDentifier) is a one type of content addressing used on the decentralized web. It was developed for
IPFS (a decentralized web protocol which we'll discuss in later tutorials), but the `CID` specification has very broad implications.
...
Many formats and protocols use content addressing already. Tools like Git and protocols like Ethereum and Bitcoin are among them, but they differ in how to interpret the data, and in the cryptographic function they use for hashing. `CID` allows us to create a universal identifier for any of these systems.
...
have cryptographic hashes which allow us to distribute and link the data.
